...
export function CustomerAccessControl({ active, id, updatedAt }: { active: boolean; id: string; updatedAt: string }) {
  ...
  return (
    <form action={formAction} aria-busy={pending} onSubmit={(event) => {
      ...
      <button className={`inline-flex min-h-11 w-full items-center justify-center whitespace-nowrap rounded-full px-5 text-[0.64rem] font-bold uppercase tracking-[0.12em] transition-[background-color,color,transform,opacity] duration-300 active:scale-[0.98] disabled:cursor-wait disabled:opacity-60 motion-reduce:transform-none ${active ? "bg-orange/12 text-orange hover:bg-orange hover:text-background" : "bg-gold text-background hover:bg-gold-light"}`} disabled={pending} type="submit">{pending ? "Updating…" : active ? "Disable account" : "Restore access"}</button>
