...
export function RefundOrderForm({ currency, orderStatus, publicReference, remainingAmountCents }: { currency: string; orderStatus: string; publicReference: string; remainingAmountCents: number }) {
  ...
  return (
    <form action={formAction} aria-busy={pending} className="space-y-4" onSubmit={(event) => {
      ...
      <button className="inline-flex min-h-11 w-full items-center justify-center whitespace-nowrap rounded-full bg-orange/12 px-5 text-[0.64rem] font-bold uppercase tracking-[0.12em] text-orange transition-[background-color,color,transform,opacity] duration-300 hover:bg-orange hover:text-background active:scale-[0.98] disabled:cursor-wait disabled:opacity-60 motion-reduce:transform-none" disabled={pending} type="submit">{pending ? "Contacting Stripe…" : rejecting ? "Reject & refund" : "Issue refund"}</button>
