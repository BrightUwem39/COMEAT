  ...
  return (
    <main className="grid min-h-[calc(100svh-5rem)] place-items-center bg-background py-8 sm:py-12" id="main-content">
      <Container className="w-full">
        <section className="mx-auto max-w-2xl border-y border-border py-8 motion-safe:animate-[hero-content-enter_700ms_cubic-bezier(0.16,1,0.3,1)_both] sm:py-10" aria-labelledby="payment-result-title">
          <div className={`mb-8 h-px w-20 ${content.accentClass}`} />
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              {result ? (
                <Link className="inline-flex min-h-11 items-center justify-center rounded-lg bg-gold px-5 text-[0.68rem] font-bold uppercase tracking-[0.12em] text-background transition-[background-color,transform,box-shadow] duration-300 hover:-translate-y-0.5 hover:bg-gold-light hover:shadow-[0_12px_30px_rgba(230,165,26,0.16)]" href={`/profile/orders/${encodeURIComponent(result.orderReference)}`}>
                  Track order
                </Link>
              ) : null}
              {content.canRetry ? <Link className="inline-flex min-h-11 items-center justify-center rounded-lg border border-border px-5 text-[0.68rem] font-bold uppercase tracking-[0.12em] text-foreground transition-colors hover:border-gold hover:text-gold" href="/checkout">← Back to payment</Link> : null}
              <Link className="inline-flex min-h-11 items-center justify-center rounded-lg border border-border px-5 text-[0.68rem] font-bold uppercase tracking-[0.12em] text-foreground transition-colors hover:border-gold hover:text-gold" href="/menu">
                Return to menu
              </Link>
            </div>
          </div>
...
function ResultDetail({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-5 border-b border-border py-4">
      <dt className="text-[0.6rem] font-bold uppercase tracking-[0.14em] text-muted">{label}</dt>
      <dd className="break-all text-right text-sm font-semibold text-foreground">{value}</dd>
    </div>
  );
}
