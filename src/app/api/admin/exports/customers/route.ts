import { buildCustomerExport, csvResponse, parseExportRange } from "@/server/admin-exports";
...
export async function GET(request: Request) {
  ...
  const csv = await buildCustomerExport(range.from, range.toExclusive);
  return csvResponse(csv, `comeat-customers-${range.fromValue}-to-${range.toValue}.csv`);
}
